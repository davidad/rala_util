...
const char* hierarchy_file = NULL;
int from_port = 0;
int cell_size = 40;
...
void set_up_drawing_environment(void) {
	...
	background_surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, height);
	background_cr = cairo_create( background_surface );

	// If hierarchy rendering has been requested, draw all required boxes underneath everything else.
	if (hierarchy_file != NULL) {
/*
		cairo_translate(background_cr, width/2-cell_size/2, height/2-cell_size/2);
		cairo_scale (background_cr, cell_size, cell_size);
		cairo_translate(background_cr, -center_x, center_y);
*/

		hierarchy_draw_boxes( background_cr, hierarchy_file );
		hierarchy_draw_boxes( cr, hierarchy_file );
	}
...
void dispatch_command(char buf) {
  ...
  if(use_frames) {
	...
	if(skin == DAY) {
		...
		next_command_char(buf, &cl, rala_glyph_set_cell_cb_scott, rala_glyph_set_arrow_cb_scott, clear_scott, normal, frame);
	}
}
...
void print_help(char** argv) {
	printf("Usage:\n\t%s [--blank=stem|hollow|none] [--skin=day|night|scott] [--to=sdl|png filename|ps filename|pdf filename|file filename|listen-on port|connect-to host port] [--from=file filename|listen-on port|connect-to host port]\n",argv[0]);
	exit(0);
}
...
int main(int argc, char **argv) {
	...
	while(i < argc) {
		...
		else if(!strcmp(argv[i], "--skin=scott")) {
			skin=SCOTT;
			blank_cell_style=NONE;
		}
		else if(!strncmp(argv[i], "--center-x=", 10)) {
			center_x=2*atof(strchr(argv[i],'=')+1);
//			if(strstr(argv[i], ".5")) {
//				center_x++;
//			}
		}
		else if(!strncmp(argv[i], "--center-y=", 10)) {
			center_y=2*atof(strchr(argv[i],'=')+1);
//			if(strstr(argv[i], ".5")) {
//				center_y++;
//			}
		}
		...
		else if(!strncmp(argv[i], "--hierarchy=", 12)) {
			hierarchy_file = argv[i]+12;
		}
	...
	if(output == TO_FILE || output == TO_SOCKET) {
		fprintf(stderr, "Sorry, this output method is not implemented. Defaulting to SDL output.\n");
		output = TO_SDL;
	}
	if(from_port == 0 && from_file == NULL) {
		from_file = stdin;
