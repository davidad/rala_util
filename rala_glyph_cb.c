...
void rala_glyph_set_cell_cb_scott(void* v, affine_t t) {
	struct cl* cl = (struct cl*)(((set_cell_cb_t*)v)->cl);
	cairo_t *cr = cl->cr;
  static cairo_pattern_t* memoized_pattern[CELL_TYPE_MAX];
  static int cell_width, cell_height; //in pixels
  ...
  if(!memoized_pattern[cell_type]) {
    ...
    switch(cell_type) {
      case CELL_TYPE_BLANK:
        break;
      case CELL_TYPE_STEM:
        stem_cell_glyph_scott(m_cr);
        break;
      case CELL_TYPE_AND:
        and_gate_glyph_scott(m_cr);
        break;
      case CELL_TYPE_OR:
        or_gate_glyph_scott(m_cr);
        break;
      case CELL_TYPE_XOR:
        xor_gate_glyph_scott(m_cr);
        break;
      case CELL_TYPE_NAND:
        nand_gate_glyph_scott(m_cr);
        break;
      case CELL_TYPE_WIRE:
        wire_cell_glyph_scott(m_cr);
        break;
      case CELL_TYPE_CROSSOVER:
        crossover_cell_glyph_scott(m_cr);
        break;
      case CELL_TYPE_COPY_E:
        copy_cell_glyph_scott(m_cr,2);
        break;
      case CELL_TYPE_COPY_N:
        copy_cell_glyph_scott(m_cr,3);
        break;
      case CELL_TYPE_COPY_W:
        copy_cell_glyph_scott(m_cr,0);
        break;
      case CELL_TYPE_COPY_S:
        copy_cell_glyph_scott(m_cr,1);
        break;
      case CELL_TYPE_DELETE_E:
        delete_cell_glyph_scott(m_cr,2);
        break;
      case CELL_TYPE_DELETE_N:
        delete_cell_glyph_scott(m_cr,3);
        break;
      case CELL_TYPE_DELETE_W:
        delete_cell_glyph_scott(m_cr,0);
        break;
      case CELL_TYPE_DELETE_S:
...
void rala_glyph_set_arrow_cb_scott(void* v, affine_t t) {
	struct cl* cl = (struct cl*)(((set_cell_cb_t*)v)->cl);
	cairo_t *cr = cl->cr;
  static cairo_pattern_t* memoized_pattern[ARROW_TYPE_MAX];
  static int cell_width, cell_height; //in pixels
  ...
  if(!memoized_pattern[arrow_type]) {
    ...
    switch(arrow_type) {
      case ARROW_TYPE_NONE:
        arrow_none_glyph_scott(m_cr);
        break;
      case ARROW_TYPE_X:
        arrow_x_glyph_scott(m_cr);
        break;
      case ARROW_TYPE_0:
        arrow_0_glyph_scott(m_cr);
        break;
      case ARROW_TYPE_1:
        arrow_1_glyph_scott(m_cr);
        break;
    }
    memoized_pattern[arrow_type] = cairo_pattern_create_for_surface(memoizer_surface);
    cairo_pattern_set_matrix(memoized_pattern[arrow_type], &scale_matrix);
